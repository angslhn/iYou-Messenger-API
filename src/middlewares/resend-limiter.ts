...
/**
 * Middleware khusus untuk membatasi laju permintaan (rate limiting) pada endpoint resend.
 * Bertujuan untuk melindungi sistem dari serangan Brute Force dan Credential Stuffing.
 * Jika sebuah IP melampaui batas percobaan resend yang ditentukan, sistem akan
 * secara otomatis memblokir akses sementara dan mengembalikan respons HTTP 429 (Too Many Requests).
 */
const resendLimiter: RateLimitRequestHandler = rateLimit({
  // Durasi jendela waktu memori/pemblokiran: beberapa waktu (dalam milidetik)
  windowMs: env.RESEND_LIMIT_DURATION_TIME,

  // Batas maksimal percobaan (request) resend per IP dalam rentang waktu windowMs
  limit: env.RESEND_LIMIT,
